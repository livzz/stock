                      ...
                      <div class="level-item">
                          <div>
                              <div>
                                  ...
                                  <input class="input" placeholder="Invoice no" v-model="challanNumber">
                              </div>
                          </div>
                      </div>
                  </div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      //  console.log(x);
        this.db.delivery_challan.insert(x, err => {
          if (err) {
    ...
    updateEvent(){
      ...
      if (this.checkFields()) {
        ...
        console.log(x);
        this.db.delivery_challan.update({_id:this._id},x, err => {
          if (err) {
            alert("Error Try Again!!", "Stock Manager");
          } else {
            this.rows = [];
...
//  }
  ...
  mounted() {
     ...
     this.$electron.ipcRenderer.on('message', (event, data) => {
        console.log(data);
        this._id = data.trim();
        this.db.delivery_challan.find({_id:data}, (err, docs) => {
        if (err) {
          ...
          //  this.selectedParty = this.findParty(d.detail.party.gstin);
            ...
            this.challanNumber = d.detail.chalanNo;
            this.y=d.detail.party;
            console.log('nam',this.y);
