...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      if (this.checkFields()) {
        ...
        this.db.deliverychallan.insert(x, err => {
          if (err) {
  ...
  created() {
    ...
    this.db.deliverychallan = new Datastore({ filename: "delivery_challan", autoload: true });
  },
  computed:{
    filterList:function(){
      return this.stocks.filter((data)=>{
