...
export default {
      ...
      if (this.checkFields()) {
        const x = {
          ...
          detail: {
            ...
            debit_number: this.data.debitNumber,
            party: this.getPartyName(),
            original_date:this.data.originalDate,
        ...
        this.db.debitNote.insert(x, err => {
          if (err) {
  ...
  created() {
    ...
    this.db.debitNote = new Datastore({ filename: "debit_note", autoload: true });
  },
 computed:{
    filterList:function(){
      return this.stocks.filter((data)=>{
